...
function formatDate(d1, d2) {
    ...
    else if (d1.getDate()+1 === d2.getDate() && d1.getUTCHours() == 0 && d2.getUTCHours() == 0)
    ...
    chrome.identity.getAuthToken({ interactive: true }, function (token) {
        ...
        Promise.all([
            ...
            .then(function (data) {
                ...
                var urls = data[0].items.map(calendar_data => fetch('https://www.googleapis.com/calendar/v3/calendars/' + encodeURIComponent(calendar_data.id) + '/events?orderBy=startTime&singleEvents=true&timeMin=' + ISODateString(today) + '&timeMax=' + ISODateString(month), init).then(res => res.json()).then(res => res.items))
                urls = urls.concat(data[1].map(course_data => fetch('https://mvla.instructure.com/api/v1/courses/' + course_data.id + '/assignments').then(res => res.text()).then(res => JSON.parse(res.toString().replace('while(1);', '')))))
                Promise.all(urls)
				    ...
				    details.appendChild(div)
                        ...
                        calendar.innerHTML='<input type="button" style="font-weight:bold" id="expand" value="Expand All"><hr>'+calendar.innerHTML
                        document.getElementById('expand').onclick = function () {
                            Array.from(document.querySelectorAll('details')).forEach(el => el.open = expandFlag)
                            expandFlag = !expandFlag;
                            cell.value = expandFlag ? "Expand All" : "Collapse All"
